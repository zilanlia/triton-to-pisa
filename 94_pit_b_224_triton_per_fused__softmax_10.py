...
def triton_per_fused__softmax_10(in_ptr0, out_ptr2, xnumel, rnumel):
    ...
    r1 = rindex
    x0 = xindex
    tmp0 = tl.load(in_ptr0 + (r1 + (arg_0_shape[2]*x0)), rmask & xmask, other=0).to(tl.float32)
    ...
    tl.store(out_ptr2 + (r1 + (arg_0_shape[2]*x0)), tmp13, rmask & xmask)

    # tmp13 = tmp0.to(tl.float32)
    # tl.store(out_ptr2 + (r1 + (arg_0_shape[2]*x0)), tmp13, rmask & xmask)
...
def call(args):
    grid=lambda meta: (triton.cdiv(arg_0_stride[0], 1024), )
    triton_per_fused__softmax_10[grid](*args, arg_0_stride[0], arg_0_stride[1])
